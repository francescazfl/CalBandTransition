...
for ib = 1:nb
    ...
    if valid_inst(ib).i_target && valid_inst(ib).j_target
        ...
        % Get the initial location of the current marcher
        ...
        if valid_inst(ib).direction
            ...
            if ~answer
                fprintf(['WARNING: Marcher %d cannot reach their target ', ...
                    '(%d, %d) from their initial position (%d, %d) by ', ...
                    'following their assigned direction: %s.\n'], ib, ...
                    i_target, j_target, i_marcher, j_marcher, direction)
            end
        end
